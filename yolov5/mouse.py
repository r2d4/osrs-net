...
def move_to_img(img_name, deviation, speed):
    ...
    if loc:
        ...
        if speed == 0:
            os.system('xdotool mousemove ' +
                      str(x_bounds) + ' ' + str(y_bounds))
...
def mouse_bez(init_pos, fin_pos, deviation, speed):
    ...
    control_1 = (init_pos[0] + choice((-1, 1)) * abs(ceil(fin_pos[0]) - ceil(init_pos[0])) * 0.01 * randint(deviation / 2, deviation),
                 init_pos[1] + choice((-1, 1)) * abs(ceil(fin_pos[1]) - ceil(
                     init_pos[1])) * 0.01 * randint(deviation / 2, deviation)
                 )
...
def move(mouse_points, draw=False, rand_err=True):
    ...
    if draw == True:
        drawpoints = [(v[0] - REL_ORIGIN[0], v[1] - REL_ORIGIN[1])
                      for v in mouse_points if type(v) is not str]
        draw_points(drawpoints, width=754, height=503)

    pyautogui.

    # round floats to ints
    mouse_points = [[round(v) for v in x] if type(
        x) is not str else x for x in mouse_points]
    for coord in mouse_points:
        if coord == 'click':
            ...
            outfile.write('xdotool mousemove ' +
